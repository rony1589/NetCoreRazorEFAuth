    ...
    public class DeleteModel(IAlbumSetService albumSetService, IMapper mapper, ILogger<DeleteModel> logger,JsonSerializerOptions jsonSerializerOptions) : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                ...
                if (albumId == 0)
                {
                    TempData["OperationResult"] = JsonSerializer.Serialize(OperationResult.Fail("Álbum no existe."), _jsonSerializerOptions);
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Ocurrió un error al Eliminar el álbum");
                _logger.LogError(ex, "Ocurrió un error al Eliminar el álbum");
                return Page();
            }
        }
    }
